...
const inFlight = new Map<string, Promise<KeywordScore>>();
...
async function computeAndStore(
  ...
  db.insert(keywordScores)
    .values({ keyword, country, popularity, difficulty, opportunity, classification, fetchedAt })
    .onConflictDoUpdate({
      target: [keywordScores.keyword, keywordScores.country],
      set: { popularity, difficulty, opportunity, classification, fetchedAt },
    })
    .run();

  return { keyword, country, popularity, difficulty, opportunity, classification, fetchedAt, stale: false };
}

function dedupedCompute(keyword: string, country: string): Promise<KeywordScore> {
  const key = `${country}:${keyword}`;
  const existing = inFlight.get(key);
  if (existing) return existing;
  const promise = computeAndStore(keyword, country).finally(() => {
    inFlight.delete(key);
  });
...
export async function scoreKeyword(
  ...
  return dedupedCompute(keyword, country);
}
