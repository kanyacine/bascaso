...
How Bascaso fetches and uses ASC API data. Based on real API exploration.
...
9. **`GET /v1/nominations` requires `filter[state]`** – the nominations list endpoint requires a `filter[state]` parameter (DRAFT, SUBMITTED, or ARCHIVED). To fetch all nominations, make three parallel requests filtering by each state and merge the results.
10. **Screenshot set `relationships` omitted** – when querying `/appScreenshotSets?include=appScreenshots`, the API returns screenshots in `included` but omits the `relationships` key on each set, making it impossible to map screenshots to their parent set. Workaround: fetch screenshots per set via `/v1/appScreenshotSets/{id}/appScreenshots`
