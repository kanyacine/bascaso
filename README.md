...
**[appscreen](https://github.com/YUZU-Hub/appscreen)** by YuzuHub is where the screenshot editor comes from. Its canvas composer was ported to TypeScript and React (`src/lib/screenshot-editor/`, `src/components/screenshot-editor/`) and then wired to the App Store Connect side of the app. How it works: [docs/SCREENSHOT-EDITOR.md](docs/SCREENSHOT-EDITOR.md).

itsyconnect-macos and appscreen are MIT. RespectASO is AGPL-3.0, which is what makes Bascaso AGPL-3.0 as well.
...
**A screenshot editor that ends on App Store Connect.** appscreen composes the image; what Bascaso adds around it is the store. The editor only offers the formats your app's platforms actually have, texts and images are held per language, and an export renders every working language and format and replaces the matching screenshot sets – or hands you a zip. Listing locales you never set up as working languages can be translated on the way out.
...
**Screenshot editor** – compose App Store screenshots in the app. Solid, gradient or image backgrounds with optional noise, the capture flat or on a rotatable 3D iPhone, headline and subheadline with per-language layout, text/emoji/icon/graphic elements on three layers, and popouts that zoom into a crop. Work in several languages at once, keep named versions of a document, and export to a zip or straight into your screenshot sets. Fonts come from the ones installed on your Mac; Google Fonts is off until you turn it on. See [docs/SCREENSHOT-EDITOR.md](docs/SCREENSHOT-EDITOR.md).
...
├── src/lib/screenshot-editor/* → canvas composer, 3D mockups, export planning
...
└── ~/Library/Application Support/bascaso/
    ...
    └── screenshot-assets/ → images imported into the screenshot editor
```

**Stack:** Electron 40 · Next.js 16 · React 19 · TypeScript · Tailwind v4 · shadcn/ui · Phosphor Icons · Geist font · SQLite via better-sqlite3 · Drizzle ORM · Recharts · dnd-kit · three.js · Zod · Vercel AI SDK · AES-256-GCM envelope encryption · macOS Keychain
...
The screenshot editor is a TypeScript and React port of [appscreen](https://github.com/YUZU-Hub/appscreen) by YuzuHub.

Added here: the on-device Apple model, the agentic keyword workflow, tiered AI routing, the managed tier, and the App Store Connect side of the screenshot editor (working formats, per-language documents, export and upload).
